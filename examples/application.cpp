...
int setNumRelays(String command);

/* This function is called once at start up ----------------------------------*/
void setup()
{
	Serial.begin(115200);
	relayController.setNumberOfRelays(8);
	relayController.setAddress(0, 0);
	Particle.function("controlRelay", triggerRelay);
	Particle.function("setNumRelays", setNumRelays);
}
...
int setNumRelays(String command){
	int nRelays = command.toInt();
	relayController.setNumberOfRelays(nRelays);
	if(nRelays > 16){
		relayController.setAddress(0,0);
	}else{
